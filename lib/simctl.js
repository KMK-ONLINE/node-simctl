...
async function simCommand (command:string, timeout:number, args:Array = [], env = {}, executingFunction = exec) {
  ...
  try {
    ...
    if (e.stderr) {
      log.errorAndThrow(`simctl error running fbsimctl '${args}': ${e.stderr.trim()}`);
    } else {
      log.errorAndThrow(e);
    }
  }
...
async function getScreenshot (udid:string):string {
  ...
  await exec('xcrun', ['simctl', 'io', udid, 'screenshot', pathToScreenshotPng]);
...
export { installApp, removeApp, launch, spawn, spawnSubProcess, openUrl, shutdown, createDevice,
         deleteDevice, eraseDevice, getDevices, getScreenshot };
